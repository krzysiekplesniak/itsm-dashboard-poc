...
const TOOL_LABELS = { itsm_status: 'checking workspace', itsm_scan: 'personal-data check', itsm_decide_privacy: 'applying privacy decision', itsm_profile: 'reading what is in the files', itsm_propose_spec: 'proposing KPIs', itsm_get_spec: 'reading spec', itsm_set_spec: 'writing spec', itsm_patch_spec: 'changing spec', itsm_build: 'computing KPIs + building dashboard', itsm_kpis: 'computing KPIs', itsm_findings: 'finding what matters', itsm_records: 'drilling into records', itsm_verify: 'verifying numbers', itsm_record_decision: 'saving decision to ORG_PROFILE', itsm_org_profile: 'reading ORG_PROFILE', itsm_brief: 'writing BRIEF.md', itsm_save_version: 'saving version' };
...
$('#files').onchange = async () => {
  const files = [...$('#files').files]; if (!files.length) return; const names = [];
  for (const f of files) {
    add('sys', `Uploading ${f.name}…`);
    const r = await fetch(`/api/files?name=${encodeURIComponent(f.name)}`, { method: 'POST', body: await f.arrayBuffer() }).then(r => r.json());
    if (r.error) { add('sys', '⚠ ' + r.error); continue; }
    names.push(r.file); add('sys', `${r.file}: ${r.rows} rows, anonymised (removed: ${r.dropped?.join(', ') || '—'}${r.onHold?.length ? '; ON HOLD: ' + r.onHold.map(c => c.column).join(', ') : ''})`);
  }
  $('#files').value = '';
  if (names.length) send(`I uploaded ${names.length} file(s): ${names.join(', ')}.`, { silent: true });
};
$('#mock').onclick = async () => {
  const r = await fetch('/api/use-sample', { method: 'POST' }).then(r => r.json());
  if (r.error) return add('sys', '⚠ ' + r.error);
  r.files.forEach(x => add('sys', `${x.file}: ${x.rows} rows (synthetic sample), anonymised`));
  send('I loaded the synthetic sample: ' + r.files.map(f => f.file).join(', ') + '.', { silent: true });
};
$('#reffile').onchange = async () => {
  const f = $('#reffile').files[0]; if (!f) return;
  const r = await fetch('/api/reference', { method: 'POST', body: await f.arrayBuffer() }).then(r => r.json()); $('#reffile').value = '';
  if (r.error) return add('sys', '⚠ ' + r.error);
  add('sys', `Compared with the report: ${r.reference.filter(x => x.withinTolerance).length} of ${r.reference.length} values within ±0.1.`);
  send('I uploaded the official report values (reference.csv). Explain the comparison.', { silent: true });
};
